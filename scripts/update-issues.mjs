...
for (const [key, project] of Object.entries(projects)) {
  ...
  for (const user of users) {
    ...
    for (const issue of await response.json()) issues.set(`${project}:${issue.iid}`, { iid: issue.iid, title: issue.title, assignees: issue.assignees.map(a => a.username), createdAt: issue.created_at, updatedAt: issue.updated_at, url: issue.web_url, high: issue.labels.some(label => label.toLowerCase() === 'high') });
  }
